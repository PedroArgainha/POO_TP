...
import Model.Exceptions.DivisaoNaoExisteException;
...
public class Casa implements Serializable {
    ...
    // Metodo auxiliar privado: obtém a divisão REAL (não clone) do mapa interno.
    ...
    public String resumoDispositivosPorDivisao() {
        StringBuilder sb = new StringBuilder();
        ...
        for (Map.Entry<String, Divisao> entry : this.divisoes.entrySet()) {
            ...
            sb.append(entry.getValue().resumoDispositivos());
        }

        return sb.toString();
    }

    public void validarAcaoSobreDispositivos(String tipoAcao, List<Integer> idsDispositivos)
            throws DispositivoNaoExisteException, NivelInvalidoException {

        for (Integer id : idsDispositivos) {
            Dispositivo d = getDispositivoPorId(id);

            switch (tipoAcao) {
                case "LIGAR":
                case "DESLIGAR":
                    // Qualquer dispositivo pode ser ligado/desligado.
                    break;

                case "NIVEL":
                    if (!(d instanceof Regulavel)) {
                        throw new NivelInvalidoException(
                                "O dispositivo '" + d.getNome() + "' não é regulável."
                        );
                    }
                    break;
                ...
                case "FECHAR":
                    if (!(d instanceof Abrivel)) {
                        throw new DispositivoNaoExisteException(
                                "O dispositivo '" + d.getNome() + "' não é abrível/fechável."
                        );
                    }
                    break;
